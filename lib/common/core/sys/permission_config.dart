    ...
    switch (status) {
      ...
      case PermissionStatus.permanentlyDenied:
        showDialog(
            ...
            builder: (context) => PermissionDialog(title: title, content: content));
        return false;
    }
  }
  ...
  Future<bool> requests(List<Permission> permissions) async {
    ...
    await Future.forEach(status.entries, (MapEntry element) {
      ...
      if (_requestTitle.isNotEmpty) {
        showDialog(
            context: Application.navigator.currentContext!,
            builder: (context) => PermissionDialog(title: 'Thông báo', content: 'Bạn cần cấp quyền truy cập ${_requestTitle.join(', ')} để tiếp tục'));
      }
    });
    return _result;
  }
